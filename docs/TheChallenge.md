...
These same unique identifiers will appear on every page that the user visits.  In order to measure the number of unique identifiers on a page or across a number of different pages, it is necessary to discount the identifier duplicates.  Obtaining an exact answer to a _COUNT DISTINCT_ query with massive data is a difficult computational challenge. It is even more challenging if it is necessary to compute arbitrary expressions across sets of unique users. For example, if set _S_ is the set of users visiting the Sports page and set _F_ is the set of users visiting the Finance page, the intersection expression _S &#8745; F_ represents the users that visited both Sports and Finance.
